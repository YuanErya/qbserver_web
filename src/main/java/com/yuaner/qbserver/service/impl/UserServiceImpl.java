...
public class UserServiceImpl implements UserService {
     ...
     */
    @Override
    public ApiResult registerUser(RegisterDTO registerDTO) {
        ...
        SimpleUser user=new SimpleUser(null, registerDTO.getUserName(),
        ...
        return ApiResult.success(user,"注册成功！");
    }
    ...
    public ApiResult<User> loginUserByPassword(LoginDTO loginDTO) {
        ...
        if(RegexUtils.isEmailInvalid(loginDTO.getUsernameOrEmail())) {
            //用户名登录
            user= userMapper.getSimpleUserByUserName(loginDTO.getUsernameOrEmail());
        }else{
            //邮箱号登录
            user= userMapper.getSimpleUserByEmail(loginDTO.getUsernameOrEmail());
        }
        if(user==null){
